...
function MainChat({date}) {
  ...
  const[img, setImg] = useState('')
      ...
      // changing images in chatroom 
      useEffect(() => {
          setImg(Math.floor(Math.random()*5000))
      }, [roomId])

      //function to send our message to our database
       const sendMessage=(e)=>{
            e.preventDefault()
           ...
           audio.play();   
           toast.success('Message Sent Successsfully',{position:toast.POSITION.TOP_CENTER}) 
           setMessage('')
       }

       //function to fetch messages from database (React Hook)
          useEffect(()=>{
                  if(roomId){
          ...
          const clearMessages=()=>{
            ...
            toast.success('Messages Cleared',{position:toast.POSITION.BOTTOM_CENTER})
            setOpen(false)        
          }
  ...
  return (
    <div className="chat-field">
            ...
            <div className="header_info">
              ...
              <p>Last seen &nbsp;
                {messages.length>0 ?(
                  ...
                  <small className="Not seen yet seen">Not seen yet</small>
                  )}   
              </p>
            </div>
        ...
        <div className="message-footer">
            <form>
              <InputEmoji value={message} onChange={setMessage} placeholder="Type a message"/>
